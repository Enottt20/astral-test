...
func NewDocumentService(repo *storage.Repository, cache *redis.Client) *DocumentService {
...
func (s *DocumentService) Upload(ctx context.Context, token, meta, jsonData string, file multipart.File, header *multipart.FileHeader, isFileLoaded bool) (*domain.Document, error) {
...
func (s *DocumentService) GetAll(ctx context.Context, token, login, key, value string, limit int) ([]*domain.Document, error) {
	cacheKey := "docs:" + token
	if login != "" {
		cacheKey += ":" + login
	}
...
func (s *DocumentService) GetByID(ctx context.Context, token, id string) (*domain.Document, []byte, error) {
	cacheKey := "doc:" + id
	if cached, err := s.cache.Get(ctx, cacheKey).Result(); err == nil {
